...
def test_analytics_computation(client, admin_user, professional_user, run_app_context):
    ...
    with run_app_context:
        ...
        t1 = Ticket(
            ...
            reporter_name=admin_user.name,
            prn=admin_user.prn or "ADMIN",
...
def test_export_csv(client, admin_user, run_app_context):
    """Test exporting tickets as CSV without pandas."""
    with run_app_context:
        b = Building(name="Export Building")
        f = Floor(level=1, name="1st Floor", building=b)
        r = Room(number="EXP101", floor=f)
        db.session.add_all([b, f, r])
        db.session.commit()
    ...
    assert "Export test issue" in response.get_data(as_text=True)


def test_export_pdf(client, admin_user, run_app_context):
    """Test exporting tickets as PDF without pandas or matplotlib."""
    with run_app_context:
        b = Building(name="Export Building 2")
        f = Floor(level=1, name="1st Floor", building=b)
        r = Room(number="EXP102", floor=f)
        db.session.add_all([b, f, r])
        db.session.commit()
